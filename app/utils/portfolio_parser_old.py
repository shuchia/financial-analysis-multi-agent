...
def parse_portfolio_output(crew_output: str, investment_amount: float) -> Dict:
    ...
    try:
        ...
        # Pattern 1: "TICKER - XX% ($X,XXX) - Full reasoning text here"
        # Captures everything after amount as reasoning (including hyphenated words)
        # Uses non-greedy match to stop at next ticker or section
        ...
        for match in matches:
            if len(match) >= 3:
                ...
                if len(match) == 5:  # Pattern 1 - simplified structure
                    ticker = match[0].upper()
                    category_in_parens = match[1] if match[1] else ""
                    percentage = float(match[2])
                    amount_str = match[3].replace(',', '')
                    full_text_after_amount = match[4].strip()

                    # Parse the text after amount - could be "Category - reasoning" or just "reasoning"
                    category = category_in_parens if category_in_parens else ""
                    reasoning_text = full_text_after_amount

                    # If there's a dash in the text, try to split category and reasoning
                    if ' - ' in full_text_after_amount and not category:
                        parts = full_text_after_amount.split(' - ', 1)
                        ...
                        category_keywords = ['Technology', 'Healthcare', 'Financial', 'Consumer', 'Energy',
                                           ...
                                           'Intermediate', 'Short', 'Long']
                        if len(parts[0].split()) <= 4 and any(keyword in parts[0] for keyword in category_keywords):
                            category = parts[0].strip()
                            reasoning_text = parts[1].strip() if len(parts) > 1 else full_text_after_amount
